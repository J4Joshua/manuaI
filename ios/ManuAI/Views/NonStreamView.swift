      ...
      VStack {
        ...
        VStack(spacing: 12) {
          ...
          CustomButton(
            ...
            style: .primary,
            isDisabled: !viewModel.hasActiveDevice || isUpdateRequired || viewModel.audioOnlyActive
          ) {
            Task {
          ...
          // ManuAI hands-free: stream the glasses mic to glasses_bridge with NO
          // camera session (the offline audio-only MVP). Not gated on
          // hasActiveDevice — the HFP mic needs Bluetooth pairing, not a DAT
          // camera. Disabled while a camera stream is live (mutually exclusive).
          CustomButton(
            title: viewModel.audioOnlyActive ? "Stop hands-free" : "Start hands-free (audio only)",
            style: viewModel.audioOnlyActive ? .destructive : .primary,
            isDisabled: viewModel.streamingStatus == .streaming
          ) {
            if viewModel.audioOnlyActive {
              viewModel.stopAudioOnly()
            } else {
